...
in
{
  options.mx.services.remote-login = {
    enable = lib.mkEnableOption "SSH driven login/unlock of the streaming session";

    createUser = lib.mkOption {
      type = lib.types.bool;
      default = true;
      description = ''
        Declare the streaming user (`mx.services.remote-desktop.user`) as a
        normal user of the system. Turn it off when the host already declares
        that user itself.
      '';
    };
    ...
    authorizedKeys = lib.mkOption {
      ...
      default = [ ];
      description = ''
        Public keys allowed to log in over SSH as the streaming user, i.e. the
        keys of the clients that send the Wake-on-LAN magic packet.
      '';
  ...
  config = lib.mkIf cfg.enable {
    assertions = [
      ...
      }
      {
        ...
        message = "mx.services.remote-login opens the graphical session Sunshine streams, which is set up by mx.services.remote-desktop.";
      }
      {
