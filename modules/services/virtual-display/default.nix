...
let
  cfg = config.mx.services.virtual-display;
...
in
{
  options.mx.services.virtual-display = {
    ...
    assertions = lib.optionals cfg.sunshine.enable [
      {
        ...
        message = "mx.services.virtual-display.sunshine.enable needs an entry in mx.services.virtual-display.displays.";
      }
      {
        assertion = config.services.sunshine.enable;
        message = "mx.services.virtual-display.sunshine.enable needs Sunshine (mx.services.remote-desktop.enable).";
      }
    ];
  };
}
